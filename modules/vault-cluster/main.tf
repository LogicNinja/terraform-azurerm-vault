...
resource "azurerm_lb" "vault_access" {
  ...
  frontend_ip_configuration {
    ...
    public_ip_address_id = azurerm_public_ip.vault_access[0].id
  }
}
...
resource "azurerm_lb_nat_pool" "vault_lbnatpool" {
  ...
  resource_group_name            = var.resource_group_name
  name                           = "ssh"
  loadbalancer_id                = azurerm_lb.vault_access[0].id
  protocol                       = "Tcp"
...
resource "azurerm_lb_rule" "vault_api_port" {
  ...
  frontend_ip_configuration_name = "PublicIPAddress"
  backend_address_pool_id        = azurerm_lb_backend_address_pool.vault_bepool[0].id
...
resource "azurerm_virtual_machine_scale_set" "vault_with_load_balancer" {
  ...
  network_profile {
    ...
    ip_configuration {
      ...
      primary                                = true
      subnet_id                              = var.subnet_id
      load_balancer_backend_address_pool_ids = [azurerm_lb_backend_address_pool.vault_bepool[0].id]
